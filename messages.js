const messages = {
  en: {
    appTitle : "Language simplifcation within Wikidata",
    appDescription : "This application presents information from Wikidata related to language simplification. ",
    message: {
      hello: '{msg} world'
    },
    overview : "Overview",
    persons: "Persons",
    outputTemplates : {
      TC1 : "The following {num} persons are active in the area of language simplifcation.",
      TC1Details : "Information about {entityLabel}",
      TC8 : "The following {num} tools are available in the area of language simplifcation.",
      TC8Details : "Details about {entityLabel}",
      TC2 : "The following {num} locations have people or organizations working on simplifcation.",
      TC2Details : "Active persons and relevant organizations in {entityLabel}",
      TC3 : "The following {num} organizations work on language simplifcation.",
      TC3Details : "Details about {entityLabel}",
    },
    personname : "Name: ",
    job: "Occupation",
    joblocation: "Job location",
    notfound : "No information found",
    german : "German",
    english : "English",
    tools : "Tools",
    homepage : "Homepage",
    furtherinformation : "Further information",
    locations : "Locations",
    location : "Location",
    searchdescription : "Search for persons, tools and places with a relation to language simplification.",
    search : "search",
    organizations : "Organizations",
  },
  de: {
    appTitle : "Sprachvereinfachung in Wikidata",
    appDescription : "Diese Anwendung präsentiert Information von Wikidata über Sprachvereinfachung.",
    message: {
      hello: '{msg} Welt'
    },
    overview : "Überblick",
    persons : "Personen",
    outputTemplates : {
      TC1 : "Die folgenden {num} Personen sind im Bereich Sprachvereinfachung aktiv.",
      TC1Details : "Informationen über {entityLabel}",
      TC8 : "Die folgenden {num} Tools sind im Bereich Sprachvereinfachung verfügbar.",
      TC8Details : "Details about {entityLabel}",
      TC2 : "An den folgenden {num} Orten gibt es Personen und Organisationen die an Sprachvereinfachung arbeiten.",
      TC2Details : "Aktive Personen und relevante Organisationen in {entityLabel}",
      TC3 : "Die folgenden {num} Organisationen arbeiten an leichter Sprache",
      TC3Details : "Details zu {entityLabel}"
    },
    personname : "Name",
    job: "Beruf",
    joblocation: "Arbeitsort"
    ,
    notfound : "Keine Informationen gefunden",
    german : "Deutsch",
    english : "Englisch",
    tools : "Werkzeuge",
    homepage : "Webseite",
    furtherinformation : "Weitere Informationen",
    locations : "Orte",
    location : "Ort",
    activepersonsAndOrganzations : "Aktive Personen und relevante Organizationen in {location}",
    searchdescription : "Suchen Sie nach Personen, Tools oder Orten mit Bezug zu Sprachvereinfachung.",
    search : "Suche",
    organizations : "Organisationen"
  }
}
